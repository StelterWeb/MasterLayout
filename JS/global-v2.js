...
jQuery(document).ready(function () {
	...
	jQuery("#stl-contact-form").submit(function(e){
		var fName = jQuery.trim(document.forms["contactform"]["fname"].value);
		var lName = jQuery.trim(document.forms["contactform"]["lname"].value);
		var email = jQuery.trim(document.forms["contactform"]["email"].value);

		var checkpoint = function(){
			if(document.forms["contactform"]["honeypot"]) return jQuery.trim(document.forms["contactform"]["honeypot"].value);
			if(document.forms["contactform"]["contactCheckpoint"]) return jQuery.trim(document.forms["contactform"]["contactCheckpoint"].value);
		}
...
/* Real Estate Page Show/Hide Functionality */
jQuery(document).ready(function($){
	$(".reg-toggle-box").click(function() {
     var txt = $(this).next(".reg-content").is(':visible') ? '+' : '-';
     $(this).children("span.toggle-span").text(txt);
...
jQuery(document).load(function() {
    ...
    if (jQuery('#stl-whats-new').length) {
		...
		// Gift Illustrator variable replacement
		if (jQuery('.GIfeature').length) {
				jQuery('a:contains("@[pgc-container]")').attr('href','#pgc-container');
		}
